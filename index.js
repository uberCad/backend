...
const spreadsheetId = require('./config').calculateSpreadsheetId;
const apiKey = require('./config').calculateApiKey;

const sessionsMiddleware = require('@arangodb/foxx/sessions');

const DOC_NOT_FOUND = errors.ERROR_ARANGO_DOCUMENT_NOT_FOUND.code;

const projects = db._collection('projects');
const snapshots = db._collection('snapshots');
const objects = db._collection('objects');
const materials = db._collection('materials');
const orders = db._collection('orders');
const users = db._collection('users');
const projectRelation = db._collection('project_relation');
...
module.context.use(function (req, res, next) {
    const exclusion = ['/get-materials-thermevo'];
    if (!req.session.uid && exclusion.indexOf(req.path) == -1 && !/docs/.test(req.path)) {
        res.throw(401, 'Unauthorized user');
    }
    next();
});
...
router.post('/calculate', function (req, res) {
    try {
        const authorization = request.get(`${hostname}_db/cad/auth/access`).body;
